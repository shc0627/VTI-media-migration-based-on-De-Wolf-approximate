...
/***************************************************************************************8*******/
/* *********************   Hachao Sun   $Date: 2024/05/27 22:14:15 $	*************************/
/***********************************************************************************************/
/***********************************************************************************************/
/***********************************************************************************************/
/***Based on scattering theory, we divide the media parameters into background*************/
/***parameters (background velocity parameter and background anisotropy parameter) ******/
/****and disturbance parameters (velocity disturbance and anisotropy parameter ************/
/****disturbance). We derive the mathematical representation of the De Wolf approximation*/
/****integral equation in the VTI medium and construct a generalized screen approximation*/
/*** (VTI-GS) operator for the VTI medium. We use a generalized screen approximation ******/
/********************operator for pre-stack migration imaging     *****************************/
/**********************************************************************************************/
/**********************************************************************************************/
/*********** Credits: WTIT(Jinlin university), Huachao Sun, shc0627@126.com,  **************/
/**********************************************************************************************/
/**********************************************************************************************/
/**********************************************************************************************/
#include <math.h>
...
char *sdoc[] = {
"									",
"  First-order over-relaxation iterative prestack migration imaging   	", 
"                        based on thin plate approximation		",
"									",
" Required Parameters:							",
...
int main ()
{
        ...
        for(is=0;is<ns;is++){
		  ...
		  for(ik=0,k=fk;ik<nk;++ik,k+=dk) {
			    for(iw=0,w=fw;iw<nw;++iw,w+=dw){
				      ...
				      kz1=1.0-pow(vbackground*k/w,2.0);
				      if(kz1>0.4){//angular range 
                                                /** Missing w*w/(v*v) under vertical wavenumber root signr*/
                                                kz1=(1.0*w*w-(1.0+2.0*epsilu_background)*vbackground*vbackground*k*k)/(1.0*w*w-2.0*(epsilu_background-deta_background)*vbackground*vbackground*k*k);
                                                if(kz1>=0){/*Remove singular values*/
                                                    if(iz<=10){
					                 ...
					                 bq1[iw][ik] = cmul(bq1[iw][ik],cshift1);
                                                    ...
                                                    phase_vti = (w*sqrt(kz1)/vbackground*pow((1.0-2.0*(epsilu_background-deta_background)*B_k0*B_k0),2.0));
                                                    velocity_disturbance=A_k0*(1.0-4.0*(epsilu_background-deta_background)*B_k0*B_k0+2.0*(1.0+2.0*epsilu_background)*(epsilu_background-deta_background)*pow(B_k0,4.0))/phase_vti;
                                                    cvd1[iw][ik]=crmul(cq1[iw][ik],velocity_disturbance);
                                                    bvd1[iw][ik]=crmul(bq1[iw][ik],velocity_disturbance);

                                                    /**epsilu perturbation in anisotropic parameters***/
                                                    epsilu_disturbance=-A_k0*A_k0*pow(B_k0,4.0)*(1.0+2.0*deta_background)/phase_vti;
                                                    ced1[iw][ik]=crmul(cq1[iw][ik],epsilu_disturbance);
                                                    bed1[iw][ik]=crmul(bq1[iw][ik],epsilu_disturbance);

                                                    /**deta perturbation in anisotropic parameters***/
                                                    deta_disturbance=-A_k0*A_k0*B_k0*B_k0*(1.0-(1.0+2.0*epsilu_background)*B_k0*B_k0)/phase_vti;
                                                    cdd1[iw][ik]=crmul(cq1[iw][ik],deta_disturbance);
                                                    bdd1[iw][ik]=crmul(bq1[iw][ik],deta_disturbance);
                                               }else{
                                                    /******Background phase shift******/
                                                    cq1[iw][ik] = cmplx(0.0,0.0);
                                                    bq1[iw][ik] = cmplx(0.0,0.0);
		  ...
		  pfa2cc(1,1,nk,nw,cdd1[0]);
	          pfa2cc(1,1,nk,nw,bdd1[0]);
                  ...
                  /********Assign 0 to the initial of the Relaxation factor*****/
                  ...
                  for(ix=0;ix<nx+pxl+pxr;++ix) {
		           for(iw=0,w=fw;iw<nw;w+=dw,++iw){
				      if(w==0.0) w=1.0e-10/dt; 
                                       ...
                                       kz2=(1.0/vbackground-1.0/ve[iz][ix])*w*dz;	
                                       cshift2=cmplx(cos(kz2),sin(kz2)); 
                                        
                                       /**Velocity perturbation in anisotropic parameters***/
                                       kz2_velocity=-w*(1.0/ve[iz][ix]-1.0/vbackground)*dz;
                                      // cshift2_velocity=cmplx(cos(kz2_velocity)-1.0,sin(kz2_velocity));
				       ...
				       cq1[iw][ix]=ix%2 ? cneg(cq1[iw][ix]) : cq1[iw][ix];
                                       cp2[iw][ix]=cmul(cq1[iw][ix],cshift2);
				       ...
				       cvd1[iw][ix]=ix%2 ? cneg(cvd1[iw][ix]) : cvd1[iw][ix];
                                       cvd_space1[iw][ix]= csub(cmul(cmul(cvd1[iw][ix],cmplx(0.0,kz2_velocity)),cshift2),cmul(cp2[iw][ix],cmplx(0.0,kz2_velocity)));
				       ...
				       cdd1[iw][ix]=ix%2 ? cneg(cdd1[iw][ix]) : cdd1[iw][ix];
                                       ...
                                       /***********************************************/ 
                                       //Back propagation Green
                                       /**Anisotropic velocity correction back propagation*/
                                       bq1[iw][ix]=crmul(bq1[iw][ix],1.0/nxfft);
				       bq1[iw][ix]=ix%2 ? cneg(bq1[iw][ix]) : bq1[iw][ix];
                                       bp2[iw][ix]=cmul(bq1[iw][ix],cshift2);

                                       /*Anisotropic velocity correction back propagation**/
				       bvd1[iw][ix]=crmul(bvd1[iw][ix],1.0/nxfft);
				       bvd1[iw][ix]=ix%2 ? cneg(bvd1[iw][ix]) : bvd1[iw][ix];
                                       bvd_space1[iw][ix]= csub(cmul(cmul(bvd1[iw][ix],cmplx(0.0,kz2_velocity)),cshift2),cmul(bp2[iw][ix],cmplx(0.0,kz2_velocity)));

                                       /*Anisotropic epsilu correction backpropagation**/
				       ...
				       bed1[iw][ix]=ix%2 ? cneg(bed1[iw][ix]) : bed1[iw][ix];
                                       ...
                                       /*Anisotropic deta correction back propagation**/
	...
	/*****the Generalized kirchhoff imaging condition****/
        ...
        imagere_VTI_Born=fopen("image_re_Dewolf.txt","w");
